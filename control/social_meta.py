"""Meta Graph publisher for Beo Facebook page + Instagram.

Never replies to comments. Tokens live in Beo Social .env — never Liba's.
...
def publish_post(
    *,
    ...
    """Publish. IG and FB captions can differ — same visual, native copy per network."""
    dry = is_dry_run(dry_run)
    ig_text = (caption_ig or caption or "").strip()
    fb_text = (caption_fb or caption or "").strip()
    ...
    if "facebook_page" in platforms and pid:
        if want_feed:
            try:
                meta_ids["facebook_feed"] = _facebook_feed(pid, fb_text, feed_asset)
            ...
            if not feed_asset:
                ...
                try:
                    meta_ids["instagram_feed"] = _instagram_publish(
                        ig, feed_asset, ig_text, stories=False
                    )
                except MetaError as exc:
